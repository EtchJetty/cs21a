...
module.exports = class Pluralchum {
  ...
  getSettingsPanel() {
    ...
    accessibilityPanel.append(
      new Settings.Switch("Enable text contrast test", "Uses the theme's default color if the proxy's contrast is too low", this.doContrastTest, (val) => {
    ...
    accessibilityPanel.append(
      new Settings.ColorPicker("Contrast test color", "The background color that proxy text will be tested against (black for dark themes, white for light themes)", this.contrastTestColour, (hex) => {
    ...
    if (!this.eula) {
      BdApi.showConfirmationModal(
        ...
        <div align="center" style={{color: "#eeeeee"}}>
          ...
          <br />
          Because of technical limitations, this data is cached on your computer between sessions. None of this data is ever shared, collected or uploaded, but you still ought to know.
          <br />
          <br />
          <b>You can clear this cache at any time in the plugin settings</b>, and unused cache data is automatically deleted after 30 days.
        </div>,
        {
          confirmText: "Gotcha",
          ...
          this.callbackIfMemberReady(
            ...
            function (member) {
              ...
              if (member.color) {
                ...
                if (!this.doContrastTest || textContrast >= this.contrastThreshold) ret.props.style = {color: member.color};
              }
            }.bind(this)
          );
        }
      ...
      function (member) {
        ...
        let pkBadge = (
          <span className="botTagCozy-3NTBvK botTag-1NoD0B botTagRegular-kpctgU botTag-7aX5WZ rem-3kT9wc">
            <PKBadge pk_id={props.message.id} onClick={(id) => this.updateMemberByMsg(id, this.getUserHash(props.message.author))} className="botText-1fD6Qk" />
          </span>
        );
        // Preferences
        ...
        if (member_colour) {
          ...
          if (!this.doContrastTest || textContrast >= this.contrastThreshold) userProps.style = {color: member_colour};
        }
  ...
  callbackIfMemberReady(props, callback) {
    ...
    if (this.profileMap[userHash]) {
      if (this.profileMap[userHash].status === "DONE" || this.profileMap[userHash].status === "UPDATING") callback(this.profileMap[userHash]);
    } else {
